...
plot_functional_boxplot <- function(FEMbasis, lowerfence, q1, median, q3, upperfence, ...){
  ...
  p_me <- surface_plot(FEM(median, FEMbasis), scene = 'scene3', coloraxis = 'coloraxis') %>%
    layout(annotations = add_ann('Median'))
  p_q1 <- surface_plot(FEM(q1, FEMbasis), scene = 'scene2', coloraxis = 'coloraxis') %>%
    layout(annotations = add_ann('Q1'))
  p_q3 <- surface_plot(FEM(q3, FEMbasis), scene = 'scene4', coloraxis = 'coloraxis') %>%
    layout(annotations = add_ann('Q3'))
  p_lf <- surface_plot(FEM(lowerfence, FEMbasis), coloraxis = 'coloraxis') %>%
    layout(annotations = add_ann('Lower fence'))
  p_uf <- surface_plot(FEM(upperfence, FEMbasis), scene = 'scene5', coloraxis = 'coloraxis') %>%
    layout(annotations = add_ann('Upper fence'))
...
plot_functional_boxplot_data <- function(FEMbasis, lowerfence, q1, median, q3, upperfence, data, ...){
  # Number of functions
  N <- dim(data)[2]
  ...
  p_lf <- surface_plot(FEM(lf, FEMbasis), coloraxis = 'coloraxis',
                       hovertemplate = hovertemp) %>%
    layout(annotations = add_ann('Lower fence', 0.7))
  p_uf <- surface_plot(FEM(uf, FEMbasis), scene = 'scene5', coloraxis = 'coloraxis',
                       hovertemplate = hovertemp) %>%
...
# Function 
functional_boxplot2.5D <- function(FEMbasis, lowerfence, q1, median, q3, upperfence, 
                                   data = NULL, outliergram = FALSE, 
                                   univariate_boxplot = FALSE, persistent = F, ...){
  # Mesh must be 2.5D 
  if(!is(FEMbasis$mesh, "mesh.2.5D"))
    stop("This function is implemented only for 2.5D mesh FEM objects")
  ...
  # Plot 
  p <- plot_functional_boxplot(FEMbasis, lowerfence, q1, median, q3, upperfence)
  ...
  if(!outliergram & !univariate_boxplot & !is.null(data)){
    fig <- plot_functional_boxplot_data(FEMbasis, lowerfence, q1, median, q3, upperfence, data)
  }
  ...
  if(outliergram){
    ...
    p2 <- plot_functional_boxplot_data(FEMbasis, lowerfence, q1, median, q3, upperfence, data)
...
functional_boxplot2.5D(FEMbasis, lf, q1, me, q3, uf)

# 2. Functional Boxplot with data
functional_boxplot2.5D(FEMbasis, lf, q1, me, q3, uf, data = data)

# 3. Functional Boxplot & Outliergram
functional_boxplot2.5D(FEMbasis, lf, q1, me, q3, uf, data = data, outliergram = T)

# 4. Surface plot & Univariate boxplot
functional_boxplot2.5D(FEMbasis, lf, q1, me, q3, uf, data = data, univariate_boxplot = T)
...
################################################################
# See exceedance
plot_functional_boxplot_data2 <- function(FEMbasis, lowerfence, q1, median, q3, upperfence, data, ...){
  # Number of functions
  N <- dim(data)[2]
  ...
  for (i in 1:N) {
    ...
    test <- data[,i]
    uf_test <- ifelse(test > uf, max(c(lf,q3,me,q1,uf, data)), min(c(lf,q3,me,q1,uf, data)))
    q3_test <- ifelse((test >= q3) & (test <= uf), max(c(lf,q3,me,q1,uf, data)), min(c(lf,q3,me,q1,uf, data)))
    q1_test <- ifelse((test <= q1) & (test >= lf), max(c(lf,q3,me,q1,uf, data)), min(c(lf,q3,me,q1,uf, data)))
    lf_test <- ifelse((test < lf), max(c(lf,q3,me,q1,uf, data)), min(c(lf,q3,me,q1,uf, data)))
  ...
  fig <- fig %>% layout(title = "Functional boxplot",
                        ...
                        scene6 = list(domain=list(x=c(0,1),y=c(0,0.5)),
                                      xaxis=axx, yaxis=axx, zaxis=axx,
                                      aspectmode='cube'),
                        coloraxis=list(colorscale='Greys', 
                                       cmin = min(c(lf,q3,me,q1,uf, data)),
                                       cmax = max(c(lf,q3,me,q1,uf, data)), 
                                       colorbar = list(nticks = 10, tickmode = 'auto',
                                                       len = 0.4, y = .75))
  )
  
  # Print
  print('Click on each surface index')
  
  # Display figure
  fig
  
}
plot_functional_boxplot_data2(FEMbasis, lf, q1, me, q3, uf, data = data)
